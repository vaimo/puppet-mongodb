...
Puppet::Type.newtype(:mongodb_user) do
  ...
  newproperty(:password_hash) do
    ...
    defaultto do
      raise Puppet::Error, "Property 'password_hash' must be set. Use mongodb_password() for creating hash." if @resource[:auth_mechanism] != :x509 && @resource[:password].nil? && (provider.database == :absent)
    end
    newvalue(%r{^\w+$})
